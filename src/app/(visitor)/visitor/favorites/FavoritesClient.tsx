...
export function FavoritesClient({ profile }: { profile: Profile }) {
  return (
    <DashboardShell role="visitor" userName={profile.full_name || profile.email} navItems={NAV_ITEMS}>
      <div className="p-6 lg:p-8 space-y-6">
        <motion.div initial={{ y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-xl bg-brand-violet/15 border border-brand-violet/30 flex items-center justify-center">
              <Heart className="w-5 h-5 text-brand-violet-light" />
            </div>
            <h1 className="font-display text-2xl font-bold text-white">Favorilerim</h1>
          </div>
          <p className="text-muted-foreground">
            Favori olarak işaretlediğiniz firmalar.
          </p>
        </motion.div>
        ...
        >
          ...
          <h2 className="font-semibold text-white mb-2">Henüz favori yok</h2>
          <p className="text-sm text-muted-foreground max-w-sm">
            AI Öneriler sayfasından firmaları favorilerinize ekleyin.
          </p>
        </motion.div>
      </div>
    </DashboardShell>
