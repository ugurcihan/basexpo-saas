...
import {
  Eye, EyeOff, UserPlus, AlertCircle,
  Settings2, Building2, UserCircle2, Check, Shield, Phone,
} from "lucide-react";
                ...
                {ROLES.map((role) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <Icon className={`w-5 h-5 ${isActive ? role.iconColor : "text-muted-foreground"}`} />
                      </div>
                      ...
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                          isActive ? "border-brand-indigo bg-brand-indigo" : "border-white/20"
                        }`}
                      >
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <span className="text-sm text-foreground">{role.label} olarak kaydoluyorsun</span>
                      </>
                    );
                  })()}
                  <button
                  ...
                  <div className="p-4 rounded-xl bg-white/3 border border-white/8 space-y-3">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <div className="relative flex-shrink-0 mt-0.5">
                        ...
                        />
                        <div className="w-5 h-5 rounded border-2 border-white/25 peer-checked:border-brand-cyan peer-checked:bg-brand-cyan/20 transition-all flex items-center justify-center">
                          {kvkkConsent && <Check className="w-3 h-3 text-brand-cyan" />}
                        </div>
                      </div>
                      <span className="text-sm text-muted-foreground leading-relaxed">
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); setKvkkOpen(true); }}
                          className="text-brand-cyan hover:text-white underline underline-offset-2 transition-colors font-medium"
                        >
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground/60">
                      <Shield className="w-3 h-3 text-brand-cyan/60" />
                      Verileriniz KVKK kapsamında güvenle işlenir ve 3. taraflarla paylaşılmaz.
                    </div>
                  </div>
