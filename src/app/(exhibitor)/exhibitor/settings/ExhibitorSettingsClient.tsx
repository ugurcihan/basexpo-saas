...
import {
  ...
  TrendingUp, User, Lock, AlertTriangle, CheckCircle2,
  Workflow,
} from "lucide-react";
import {
...
function SectionCard({ icon: Icon, title, children, colorClass = "bg-brand-cyan/15 border-brand-cyan/30 text-brand-cyan" }: {
...
export function ExhibitorSettingsClient({ profile }: Props) {
    ...
    <DashboardShell role="exhibitor" userName={profile.full_name || profile.email} navItems={EXHIBITOR_NAV}>
      <div className="p-6 lg:p-8 space-y-6">
        ...
        <motion.div initial={{ y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }}>
          <SectionCard icon={Lock} title="Şifre Değiştir" colorClass="bg-brand-violet/15 border-brand-violet/30 text-brand-violet-light">
            <div className="space-y-4">
              <div className="space-y-1.5">
              ...
              <Button variant="outline" onClick={handlePasswordChange} disabled={isPending || !newPw || !confirmPw} className="w-full border-brand-violet/30 text-brand-violet-light hover:bg-brand-violet/10">
